...
export function SidebarUserNav({ user }: { user: User }) {
  ...
  const userName = data?.user?.name ?? (isGuest ? "Guest" : data?.user?.email);
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            {status === "loading" ? (
              ...
              >
                {userImage ? (
                  <div className="relative size-5 shrink-0 overflow-hidden rounded-full ring-1 ring-sidebar-border/50">
                    <Image
                      alt={userName ?? "User"}
